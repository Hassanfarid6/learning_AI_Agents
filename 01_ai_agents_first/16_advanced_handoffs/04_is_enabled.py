...
weather_agent: Agent = Agent(
    ...
    model=llm_model,
    # Agar condition "is_enabled" == False → to planner_agent ko call karega (fallback/default).
    # Agar condition "is_enabled" == True  → to news_agent ko call karega.
    # handoff(..., is_enabled=...) ❌ ab kaam nahi karta qk ab "is_enabled" ko hata diya hai.
    # handoffs=[handoff(agent=news_agent, is_enabled=news_region("us-east-1")), planner_agent]
    handoffs=[handoff(agent=news_agent), planner_agent]
)

# is_enabled mein hum context ke basis pe condition laga sakte hain.

# async def main():
#     context = {"region": "us-east-1"}

#     if context["region"] == "us-east-1":
#         # ✅ sidha news_agent ko call
#         result = await Runner.run(news_agent, "Give me the latest news", context=context)
#     else:
#         # ✅ warna planner_agent ko call
#         result = await Runner.run(planner_agent, "Help me plan my week", context=context)
